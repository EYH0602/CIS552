...
instance ParseRecord Weather where
  ...
  parseRecord row = Just Weather <**> row !? "DY" <**> row !? "MAX" <**> row !? "MIN" <**> row !? "DEP"
...
splitHeader = reverse . f "" []
  where
    f acc xs "" = acc : xs
    f acc xs (',' : s) = f "" (acc : xs) s
    f acc xs (c : s) = f (c : acc) xs s
...
splitRow header row = zip header (splitHeader row)
...
Nothing <**> _ = Nothing
_ <**> Nothing = Nothing
Just f <**> Just s = parseField s >>= Just . f
...
parseEventChar '2' = Just IntenseFog
parseEventChar '3' = Just Thunder
parseEventChar '4' = Just Ice
parseEventChar '5' = Just Hail
parseEventChar '6' = Just FreezingRain
parseEventChar '7' = Just Duststorm
parseEventChar '8' = Just Smoke
parseEventChar '9' = Just BlowingSnow
...
instance ParseField Events where
  parseField ss = Just Events {getEvents = foldr f [] ss}
    where
      f x acc = case parseEventChar x of
        Nothing -> acc
        Just event -> event : acc
